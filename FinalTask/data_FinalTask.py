...
class FinalTaskData:
    ...
    def __init__(self):
        ...
        self.file_dir = u'depth_data_0.txt'
        self.num_lines = 0
        self.num_files = 0
        self.merge = []
        self.merge_train = []
    ...
    def labelCreate(self):
        ...
        f = open(self.directory + self.file_dir , 'r')
        for i in xrange(0, (self.num_files - 5)/5):
    ...
    def DataCreate(self):
        ...
        self.line_count()
        self.file_count()
        for i in range (0,(self.num_files - 5), 5):
            for count in range (0,4):
                #print i
                self.file_dir = 'depth_data_%d.txt' %(i + count)
                a = self.arrayCreate()
                self.merge_train.append(a)
                #print data_raw.shape
        
            self.file_dir = 'depth_data_%d.txt' %(i + 4)
            a = self.arrayCreate()
